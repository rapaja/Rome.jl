@testset "LTI dynsys tests" begin
    ...
    @testset "Differintegrator tests" begin
        N = 10
    ...
    @testset "Basic compound systems tests" begin
        
        s = Romeo.LTI.Diff(1)

        @testset "System scaling tests" begin
            @test 2*s == Romeo.LTI.ScaledSystem(2, s)
            @test s*2 == Romeo.LTI.ScaledSystem(2, s)
